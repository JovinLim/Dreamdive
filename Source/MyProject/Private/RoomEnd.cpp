...
// Called every frame
void ARoomEnd::Tick(float DeltaTime)
{
	Super::Tick(DeltaTime);

}

//Spawn a new "room" with all assets in randomized locations
void ARoomEnd::SpawnFloor(bool IsFirst)
{
	...
	if (!IsFirst)
	{
		ABaseRoom* LastRoom = LevelList.Last();
		SpawnLocation = LastRoom->GetSpawnLocation()->GetComponentTransform().GetTranslation();
	}

	ABaseRoom* NextRoom = nullptr;
	NextRoom = GetWorld()->SpawnActor<ABaseRoom>(TestRoom,
	...
	RandomMatrix = ARoomEnd::GenerateRMatrix(XTiles, YTiles);
	...
	AStaticFurniture_1x1* TestNextPocket = nullptr;;
	TestNextPocket = GetWorld()->SpawnActor<AStaticFurniture_1x1>(PocketObject,
	...
	for (int i = 0; i < RandomMatrix.Num(); i++) {
		...
		for (int j = 0; j < RandomMatrix[i].Num(); j++) {
			...
			if (RandomMatrix[i][j] >= 100000) {
				...
				AStaticFurniture_1x1* NextCirc = nullptr;;
				NextCirc = GetWorld()->SpawnActor<AStaticFurniture_1x1>(CirculationSpace,
			...
			if (RandomMatrix[i][j] >= 20000) {
				...
				FVector Offset = 2 * FVector(i * FloorSize.X / XTiles , j * FloorSize.Y / YTiles, 10);
				AStaticFurniture_1x1* NextPocket = nullptr;;

				if (RandomPocket == 1) {

					NextPocket = GetWorld()->SpawnActor<AStaticFurniture_1x1>(PocketObject,
						SpawnLocation + Offset, SpawnRotation, SpawnInfo);
					...
					NextPocket = GetWorld()->SpawnActor<AStaticFurniture_1x1>(PocketObject1,
						SpawnLocation + Offset, SpawnRotation, SpawnInfo);
					/*FVector NewScale = NextPocket->GetActorScale3D();
					NewScale = NewScale * FVector(float(PocketHeight) / 2, float(PocketWidth) / 2, 1);
					...
					NextPocket = GetWorld()->SpawnActor<AStaticFurniture_1x1>(PocketObject2,
						SpawnLocation + Offset, SpawnRotation, SpawnInfo);
					/*FVector NewScale = NextPocket->GetActorScale3D();
					NewScale = NewScale * FVector(float(PocketHeight) / 2, float(PocketWidth) / 2, 1);
					...
					NextPocket = GetWorld()->SpawnActor<AStaticFurniture_1x1>(PocketObject3,
						SpawnLocation + Offset, SpawnRotation, SpawnInfo);
					/*FVector NewScale = NextPocket->GetActorScale3D();
					NewScale = NewScale * FVector(float(PocketHeight) / 2, float(PocketWidth) / 2, 1);
					...
					NextPocket = GetWorld()->SpawnActor<AStaticFurniture_1x1>(PocketObject4,
						SpawnLocation + Offset, SpawnRotation, SpawnInfo);
					/*FVector NewScale = NextPocket->GetActorScale3D();
					NewScale = NewScale * FVector(float(PocketHeight) / 2, float(PocketWidth) / 2, 1);
					...
					NextPocket = GetWorld()->SpawnActor<AStaticFurniture_1x1>(PocketObject5,
						SpawnLocation + Offset, SpawnRotation, SpawnInfo);
					/*FVector NewScale = NextPocket->GetActorScale3D();
					NewScale = NewScale * FVector(float(PocketHeight) / 2, float(PocketWidth) / 2, 1);
	...
	if (NextRoom)
	{
		if (NextRoom->GetTrigger())
		{
			NextRoom->GetTrigger()->OnComponentBeginOverlap.
				AddDynamic(this, &ARoomEnd::OnOverlapBegin);
		}
	}
...
void ARoomEnd::OnOverlapBegin(UPrimitiveComponent* OverlappedComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
...
TArray<TArray<int32>> ARoomEnd::GenerateRMatrix(int RoomWidth, int RoomLength)
{
	...
	CirculationCoordinates = ARoomEnd::GenerateCirculation({ 0, 0 }, {RoomWidth-1, RoomLength-1}, RoomWidth, RoomLength, 0);
	...
	TMap<int32,Pocket> PocketList = ARoomEnd::GeneratePockets(EmptyCoordinateList);
...
TSet<TArray<int32>> ARoomEnd::GenerateCirculation(TArray<int32> EntranceCoords, TArray<int32> ExitCoords, int RoomWidth, int RoomLength, int CirculationType = 0)
{
	...
	while (!CirculationComplete)
	{
		...
		*pROffset = theta - phi;
		...
		//randOffset = ARoomEnd::OffsetCorrection(&randOffset);
...
TMap<int32, Pocket> ARoomEnd::GeneratePockets(TSet<TArray<int32>> Emptycoords)
{
	//Debugging
	...
	ARoomEnd::RemoveSubrectangles(PocketList);
...
int ARoomEnd::OffsetCorrection(float * offset) {
...
void ARoomEnd::RemoveSubrectangles(TMap<int32, Pocket> PocketList) {
